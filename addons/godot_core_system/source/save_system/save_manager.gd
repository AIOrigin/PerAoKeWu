...
func register_saveable_node(node: Node) -> void:
	...
	if _pending_node_states.has(node_path):
		CoreSystem.logger.debug("发现节点 %s 的缓存状态，正在应用..." % node_path)
		...
		if node.has_method("load_data"):
			...
			CoreSystem.logger.warning("节点 %s 缺少 load_data 方法，无法应用缓存状态！" % node_path)
...
func _get_encryption_key() -> String:
	# 从配置中获取密钥
	var key = CoreSystem.get("config_manager").get_value("save_system", "encryption_key", "")

	# 如果配置中没有密钥，生成一个默认的
	if key.is_empty():
		key = _generate_default_key()
		CoreSystem.get("config_manager").set_value("save_system", "encryption_key", key)
...
func _apply_node_states(nodes: Array) -> void:
	for node_data : Dictionary in nodes:
		...
		if node_path.is_empty():
			CoreSystem.logger.error("节点路径为空！%s" % str(node_data))
			continue
		...
		if node:
			...
			# 节点不存在，缓存数据
			CoreSystem.logger.debug("节点 %s 尚未加载，缓存其状态数据。" % node_path)
