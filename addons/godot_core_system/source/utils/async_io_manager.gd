...
func _connect_signals() -> void:
	...
	if not _io_thread or not is_instance_valid(_io_thread):
		CoreSystem.logger.error("AsyncIO: Cannot connect signals, IO thread is not initialized.")
		return
...
func _execute_read_operation(path: String, key_bytes: PackedByteArray) -> Variant:
	...
	if not file:
		CoreSystem.logger.error("AsyncIO: Failed to open file for reading: %s, Error: %s" % [path, FileAccess.get_open_error()])
		return null
...
func _execute_write_operation(path: String, data: Variant, key_bytes: PackedByteArray) -> bool:
	...
	if not dir.dir_exists(dir_path):
		...
		if err != OK:
			CoreSystem.logger.error("AsyncIO: Failed to create directory: %s, Error code: %d" % [dir_path, err])
			return false
	
	# Open and write file
	var file = FileAccess.open(path, FileAccess.WRITE)
	if not file:
		CoreSystem.logger.error("AsyncIO: Failed to open file for writing: %s, Error: %s" % [path, FileAccess.get_open_error()])
		return false

	var ok := file.store_buffer(processed_bytes)
	if not ok:
		CoreSystem.logger.error("AsyncIO: Failed to write file: %s" % path)
	file.close()
	return true
...
func _execute_delete_operation(path: String) -> bool:
	if not FileAccess.file_exists(path):
		CoreSystem.logger.warning("AsyncIO: File to delete does not exist: %s" % path)
		return true
	
	var dir = DirAccess.open(path.get_base_dir())
	if not dir:
		CoreSystem.logger.error("AsyncIO: Could not access directory for deletion: %s" % path.get_base_dir())
		return false

	var err := dir.remove(path.get_file())
	if err != OK:
		CoreSystem.logger.error("AsyncIO: Failed to delete file: %s, Error code: %d" % [path, err])
		return false
...
func _get_file_list(directory_path: String) -> Array:
	...
	if not dir:
		CoreSystem.logger.error("AsyncIO: Cannot access directory: %s" % directory_path)
		return []
...
func _process_data_for_write(data: Variant, key_bytes: PackedByteArray) -> PackedByteArray:
	...
	if not _serializer:
		CoreSystem.logger.error("AsyncIO: No serialization strategy set.")
...
func _on_task_completed(result_dict: Dictionary, internal_task_id: int) -> void: # 注意：这里接收 int ID
	...
	if public_task_id.is_empty():
		CoreSystem.logger.error("AsyncIO: Received completed signal for unknown internal task ID: %d" % internal_task_id)
		return # 无法处理，直接返回
	...
	if not result_dict is Dictionary or not result_dict.has("success"):
		CoreSystem.logger.error("AsyncIO: Internal task result format error for task %s (Internal ID: %d): %s" % [public_task_id, internal_task_id, str(result_dict)])
	...
	CoreSystem.logger.info("AsyncIO: Task %s completed (Internal ID: %d). Success: %s" % [public_task_id, internal_task_id, str(success)])
	# 发出公共信号，使用 String ID
