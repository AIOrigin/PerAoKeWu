...
func reset_config() -> void: # 方法名也改为 reset_config 更准确
	if _config_file.get_sections().is_empty():
		...
		CoreSystem.logger.debug("Config is already empty, reset_config skipped.")
		return
	...
	CoreSystem.logger.info("Config cleared. Listening systems can now apply defaults if needed.")
