...
func _ready() -> void:
	# 处理此设备首次打开这个项目
	#CoreSystem.get("config_manager").set_value("config_setup", "ok!", false)
...
func entering_the_game() -> void:
	...
	CoreSystem.get("config_manager").load_config()
	# 处理此设备首次打开这个项目
	var no_ok:bool =(not CoreSystem.get("config_manager").has_section("config_setup")
		or not CoreSystem.get("config_manager").has_key("config_setup", "ok!")
		or not CoreSystem.get("config_manager").get_value("config_setup", "ok!", false))
	var version_err:bool = (not CoreSystem.get("config_manager").has_key("config_setup", "version")
		or CoreSystem.get("config_manager").get_value("config_setup", "version", VERSION_ERR) != ProjectSettings.get_setting("application/config/version")
	)
	if no_ok or version_err:
		# 备份当前配置文件（如果存在）
		backup_config_file(CoreSystem.get("config_manager").config_path)
...
func first_time_entering_the_game() -> void:
	# config_setup
	CoreSystem.get("config_manager").set_value("config_setup", "ok!", true)
	#var version = ProjectSettings.get_setting("application/config/version", null)
	CoreSystem.get("config_manager").set_value("config_setup", "version", ProjectSettings.get_setting("application/config/version", null))
	CoreSystem.get("config_manager").set_value("config_setup", "setup_datetime_utc", Time.get_datetime_dict_from_system(true))
	CoreSystem.get("config_manager").set_value("config_setup", "setup_datetime_local", Time.get_datetime_dict_from_system(false))


## 确认配置  保存到文件
func confirm_config() -> void:
	CoreSystem.get("config_manager").save_config()


## 重制配置从文件到实例
func config_file_to_instance() -> void:
	CoreSystem.get("config_manager").reset_config()
	CoreSystem.get("config_manager").load_config()
...
func backup_config_file(config_path: String) -> void:
	...
	if CoreSystem.get("config_manager").has_section("config_setup") and CoreSystem.get("config_manager").has_key("config_setup", "version"):
		version_str = CoreSystem.get("config_manager").get_value("config_setup", "version", VERSION_ERR)
...
func restore_backup(backup_path: String) -> bool:
	if not FileAccess.file_exists(backup_path):
		...
		return false
	if DirAccess.copy_absolute(backup_path, CoreSystem.get("config_manager").config_path) != OK:
