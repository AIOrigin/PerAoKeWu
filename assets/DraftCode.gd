...
# 分辨率
	if display_mode_dic.key == "Windowed":
		...
		if resolution_mode_dic.key == "Default":
			aspect_ratios_config = CoreSystem.get("config_manager").get_value(GRAPHICS, WINDOWED_DEFAULT_ASPECT_RATIOS, MATCHING_NATIVE)
			resolutions_config = CoreSystem.get("config_manager").get_value(GRAPHICS, WINDOWED_DEFAULT_RESOLUTIONS, MATCHING_NATIVE)
...
p_default_dic: Dictionary = graphics_p_default_dic
	...
	return CoreSystem.get("config_manager").get_value(GRAPHICS, option_key, p_default_dic[p_default_key])
...
#):
	#return get_value_in_dic(
		#option_key,
		#dic,
		#CoreSystem.get("config_manager").get_value(GRAPHICS, option_key, p_default_dic[p_default_key]),
		#child_key,
		#p_default_dic)
