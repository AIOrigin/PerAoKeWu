#!/usr/bin/env python3
"""Read ASSET_VERSION from capybara_web_config.gd and cache-bust the Web HTML."""
...
CONFIG_REL = "assets/maps/route_levels/capybara_rush/capybara_web_config.gd"
...
FETCH_PATCH = """
		...
		<script>
window.__CAPYBARA_ASSET_VER = {ver_js};
(function () {{
	const VER = window.__CAPYBARA_ASSET_VER;
...
\t\t\t\treturn Promise.reject(new Error(`Failed loading file '${file}'`));
\t\t\t}
\t\t\t// read wasm as ArrayBuffer to avoid Response.clone / instantiateStreaming hangs
...
def read_asset_version(root: Path | None = None) -> str:
    ...
    if not m or not m.group(1).strip():
        raise SystemExit(f"ASSET_VERSION not found: {root / CONFIG_REL}")
    return m.group(1).strip()


# Godot 导出会生成默认 index.png（引擎 Logo）；封面必须用独立文件名，否则 CDN immutable 会一直命中旧图
COVER_REL = "assets/maps/route_levels/capybara_rush/ui/loading_cover.png"
...
SPLASH_CSS = """
body, #status {
	background: linear-gradient(180deg, #ffd6e8 0%, #e8dff0 55%, #d8cff0 100%) !important;
	color: #563266;
}
#status {
	visibility: visible;
...
#status-progress::-webkit-progress-value {
	background: linear-gradient(90deg, #ff8fb3, #ff6b9a);
	border-radius: 999px;
}
...
def patch_js(js_path: Path) -> str | None:
    ...
    if not m:
        if f'"{WASM_CANONICAL}"' not in text:
            raise SystemExit(f"{js_path}: Godot wasm template filename not found")
    else:
        template_name = m.group(0)
        if template_name != WASM_CANONICAL:
            text2 = text.replace(f'"{template_name}"', f'"{WASM_CANONICAL}"')
            if text2 == text:
                raise SystemExit(f"{js_path}: failed to rewrite {template_name} -> {WASM_CANONICAL}")
            text = text2
            print(f"index.js wasm path {template_name} -> {WASM_CANONICAL}")
    if PRELOADER_LOAD_FETCH_OLD in text:
        text = text.replace(PRELOADER_LOAD_FETCH_OLD, PRELOADER_LOAD_FETCH_NEW, 1)
    elif "read wasm as ArrayBuffer" not in text:
        raise SystemExit(f"{js_path}: Preloader.loadFetch not found; cannot patch wasm-instantiate")
    if DO_INIT_OLD in text:
        text = text.replace(DO_INIT_OLD, DO_INIT_NEW, 1)
    elif "wasmSource" not in text:
        raise SystemExit(f"{js_path}: Engine.doInit clone logic not found")
    if INSTANTIATE_WASM_OLD in text:
        ...
        raise SystemExit(f"{js_path}: instantiateWasm not found")
    js_path.write_text(text, encoding="utf-8")
    print("index.js: wasm ArrayBuffer instantiate + doInit without clone")
...
def install_wasm_alias(out_dir: Path, template_name: str | None) -> None:
    ...
    if (not src.is_file() or src.stat().st_size < 1024) and dst.is_file() and dst.stat().st_size > 1024:
        ...
        print(f"index.wasm was empty; restored from {dst.name} ({src.stat().st_size} bytes)")
    if not src.is_file() or src.stat().st_size < 1024:
        raise SystemExit(f"valid {src} not found; cannot write wasm alias")
    if not dst.exists() or dst.stat().st_size != src.stat().st_size:
        dst.write_bytes(src.read_bytes())
        print(f"wasm alias -> {dst.name}")
...
def install_loading_cover(out_dir: Path, root: Path | None = None) -> tuple[Path, str]:
    ...
    if not src.is_file() and fallback.is_file():
        src = fallback
        print(f"cover missing; using existing {fallback}")
    if not src.is_file():
        src = out_dir / "index.png"
        print(f"cover missing; using {src}")
    if not src.is_file():
        raise SystemExit(f"cover image not found: {root / COVER_REL}")
    data = src.read_bytes()
    ...
    print(f"cover -> {dst} (md5={digest})")
    return dst, digest


def _strip_wake_lock_block(text: str) -> str:
    text = re.sub(
        r"\t/\* CAPYBARA_WAKE_LOCK \*/[\s\S]*?\t/\* /CAPYBARA_WAKE_LOCK \*/\n?",
        "",
        text,
        count=1,
    )
    # 兼容旧版无标记重复注入
    while "_capyWakeLock" in text:
        new_text = re.sub(
            r"\n\tlet _capyWakeLock = null;[\s\S]*?"
        ...
        if new_text == text:
            break
        text = new_text
    text = text.replace("\t\t_capyReleaseWakeLock();\n\t\t_capyReleaseWakeLock();\n", "\t\t_capyReleaseWakeLock();\n")
    text = text.replace(
        "\t\tsetStatusNotice(_capyLoadNotice);\n\t\t_capyRequestWakeLock();\n\t\tsetStatusNotice(_capyLoadNotice);\n\t\t_capyRequestWakeLock();\n",
        "\t\tsetStatusNotice(_capyLoadNotice);\n\t\t_capyRequestWakeLock();\n",
    )
    return text
...
def _patch_html_loader_shell(text: str, total_mb: int) -> str:
    ...
    wake_block = f"""
\t/* CAPYBARA_WAKE_LOCK */
\tlet _capyWakeLock = null;
\tconst _capyLoadNotice = {notice!r};
\tasync function _capyRequestWakeLock() {{
\t\ttry {{
\t\t\tif ('wakeLock' in navigator) {{
\t\t\t\t_capyWakeLock = await navigator.wakeLock.request('screen');
\t\t\t}}
\t\t}} catch (e) {{}}
\t}}
\tfunction _capyReleaseWakeLock() {{
\t\tif (_capyWakeLock) {{
\t\t\t_capyWakeLock.release().catch(function () {{}});
\t\t\t_capyWakeLock = null;
\t\t}}
\t}}
...
\t\t}}
\t\tif (document.hidden) {{
\t\t\tsetStatusNotice('Keep this page in the foreground while downloading');
\t\t}} else {{
\t\t\tsetStatusNotice(_capyLoadNotice);
\t\t\t_capyRequestWakeLock();
\t\t}}
...
"""
    ...
    if anchor not in text:
        raise SystemExit("index.html: displayFailureNotice not found; cannot inject loader patch")
    text = text.replace(anchor, wake_block + anchor, 1)
    if "_capyLoadNotice" not in text.split("setStatusMode('progress')")[0]:
        pass
    progress_line = "\t\tsetStatusMode('progress');"
    progress_patch = "\t\tsetStatusNotice(_capyLoadNotice);\n\t\t_capyRequestWakeLock();\n\t\tsetStatusMode('progress');"
    ...
    notice_patch = "\t\t_capyReleaseWakeLock();\n\t\tsetStatusMode('notice');\n\t\tinitializing = false;"
...
def patch_html(html_path: Path, version: str) -> None:
    ...
    if "__CAPYBARA_ASSET_VER" in text:
        text, n = re.subn(
        ...
        if n != 1:
            text = re.sub(
                r'<script>\s*window\.__CAPYBARA_ASSET_VER[\s\S]*?</script>',
                patch.strip(),
                text,
                count=1,
            )
        ...
        if "</head>" not in text:
            raise SystemExit(f"{html_path}: missing </head>; cannot write cache-bust script")
    ...
    engine_expose = "const engine = new Engine(GODOT_CONFIG);\nwindow.__capyEngine = engine;"
    if "window.__capyEngine" not in text:
    ...
    style_patch = SAFE_AREA_CSS + "\n/* CAPYBARA_LOADING_COVER */\n" + SPLASH_CSS
    if "/* CAPYBARA_SAFE_AREA */" in text:
        text = re.sub(
            r"\n/\* CAPYBARA_SAFE_AREA \*/[\s\S]*?(?=\t\t</style>|</style>)",
            style_patch,
            text,
            count=1,
        )
    elif "/* CAPYBARA_LOADING_COVER */" in text:
        text = re.sub(
            r"\n/\* CAPYBARA_LOADING_COVER \*/[\s\S]*?(?=\t\t</style>|</style>)",
            style_patch,
            text,
            count=1,
        )
    ...
    print(f"wrote asset version {version} shell cache {shell_ver} cover {cover_ver} -> {html_path}")
...
def main() -> None:
    ...
    if args.cmd == "dir":
        if args.target is None:
            raise SystemExit("dir mode requires web output directory")
        patch_web_dir(args.target, version)
        return
    if args.target is None:
        raise SystemExit("html mode requires index.html path")
