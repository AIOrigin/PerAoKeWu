...
def install_wasm_alias(out_dir: Path, template_name: str | None) -> None:
    ...
    if not src.is_file():
        raise SystemExit(f"找不到 {src}，无法写入 wasm 别名")
...
def install_loading_cover(out_dir: Path, root: Path | None = None) -> tuple[Path, str]:
    ...
    src = root / COVER_REL
    if not src.is_file():
        raise SystemExit(f"找不到加载封面：{src}")
    data = src.read_bytes()
...
\t\t}}
\t\tif (document.hidden) {{
\t\t\tsetStatusNotice('Downloading — keep this page open; background tabs may pause');
\t\t}} else {{
\t\t\tsetStatusNotice(_capyLoadNotice);
\t\t\t_capyRequestWakeLock();
\t\t}}
