...
func _build_ui() -> void:
	...
	params.add_child(_make_label("Max Stair Height"))
	...
	_obstacle_panel.add_child(_make_label("Pool Triple Jump"))
	...
	help.text = "1. Track → 2. Obstacles → Ctrl+P playtest · Ctrl+S save · Tab switch · F overview"
	v.add_child(help)
...
func _set_edit_mode(mode: String) -> void:
	...
	if _phase_hint:
		if is_track:
			_phase_hint.text = "1. Set track length, theme, cliffs. Curves are generated automatically."
			_phase_hint.add_theme_color_override("font_color", Color(1.0, 0.95, 0.45))
		else:
			_phase_hint.text = "2. Place setpieces on the curve. LMB place/drag, RMB delete."
...
func _frame_whole_track() -> void:
	...
	_flash_status("Overview · Ctrl+scroll to zoom")
...
func _update_status() -> void:
	...
	_status.text = "[%s]%s d=%.0f/%.0f · obstacles%d · jumps%d · next save %s\nWheel/A·D · Tab · Ctrl+P playtest · Ctrl+S save · F overview" % [
		phase,
		flag,
		_cursor_d,
...
func _add_jump_at_cursor() -> void:
	...
	_refresh_markers()
	_refresh_list()
	_flash_status("Added pool triple jump @ %.0fm" % float(item.get("dist", 0.0)))
...
func _playtest_layout() -> void:
	...
	if entry.is_empty():
		_flash_status("Playtest failed: cannot write draft")
		return
...
func _load_custom_by_list_index(index: int) -> void:
	...
	_sync_ui_from_cfg()
	_rebuild_path()
	_flash_status("Loaded %s (saving again creates next level)" % String(cfg.get("name", level_id)))
