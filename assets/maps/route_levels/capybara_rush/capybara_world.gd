...
const JUMP_CHALLENGE_PAD_HALF_LAT := 0.78
...
func _update_jump(delta: float) -> void:
	...
	if _host._air_y < -4.0:
		if _host._game_mode == MODE_STACK:
			...
			_host._fail_game("Fell off the cliff — run over")
		return
...
func _finish_cliff_tip_rescue() -> void:
	...
	if _host._hud_tip != null and _host._playing and not _host._finished:
		_host._hud_tip.text = "Missed the trampoline · dropped a few · use a pad next time"
