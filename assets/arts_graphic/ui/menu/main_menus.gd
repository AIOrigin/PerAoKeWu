...
const DEFAULT_3D_GAME_SCENE := "res://assets/maps/levels/level_001_test_sandbox.tscn"
const RUNNER_60S_SCENE := "res://assets/maps/route_levels/runner_60s/route_runner_60s.tscn"
...
var MainMenu: Array = [ # 0--6
	...
	{"name": "Enter 3D Game", "hint": "进入原本的3D测试沙盒", "func": _on_enter_3d_game_button_pressed},
	{"name": "星火信使 · 星图", "hint": "选择星球开始跑酷送货", "func": _on_enter_galaxy_map_button_pressed},
...
func _ready() -> void:
	...
	self.visible = false
	option_window.visible = self.visible
...
func _on_enter_3d_game_button_pressed() -> void:
	_change_game_scene(DEFAULT_3D_GAME_SCENE)

func _on_enter_runner_game_button_pressed() -> void:
	_change_game_scene(RUNNER_60S_SCENE)

func _on_enter_galaxy_map_button_pressed() -> void:
	_change_game_scene(GALAXY_MAP_SCENE)

func _on_to_galaxy_map_button_pressed() -> void:
	_change_game_scene(GALAXY_MAP_SCENE)

func _change_game_scene(scene_path: String) -> void:
	if not ResourceLoader.exists(scene_path):
		push_error("Scene does not exist: %s" % scene_path)
		return
	close_menu()
	Global.change_game_scene(scene_path)
