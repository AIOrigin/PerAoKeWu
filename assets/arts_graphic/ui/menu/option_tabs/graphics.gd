...
func apply_config(apply_all: bool = false, reset_ui: bool = false) -> void:
	...
	if is_default_resolutions:
		...
		if check_screen_size != Vector2.ZERO:
			...
			CoreSystem.get("config_manager").set_value(GRAPHICS, default_aspect_ratios_key, closest_resolution.ratio)
			CoreSystem.get("config_manager").set_value(GRAPHICS, default_resolutions_key, closest_resolution.resolution)
	...
	if is_windowed_custom:
		var custom_resolutions_config = CoreSystem.get("config_manager").get_value(GRAPHICS, WINDOWED_CUSTOM_RESOLUTIONS, MATCHING_NATIVE_75)
		var check_screen_size: Vector2 = Vector2.ZERO
		...
		if check_screen_size > Vector2.ZERO:
			var closest_resolution: Dictionary = find_closest_resolution(check_screen_size)
			main_window.size = closest_resolution.size
			CoreSystem.get("config_manager").set_value(GRAPHICS, WINDOWED_CUSTOM_RESOLUTIONS, closest_resolution.size)
	# 全屏时原生
...
func get_aspect_ratios_config_value_in_dic(config_key: String) -> Dictionary:
	...
	var value = CoreSystem.get("config_manager").get_value(GRAPHICS, config_key, preset_value)
...
func _on_display_mode_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, DISPLAY_MODE, display_mode.get_main_value())
...
func _on_resolution_mode_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, RESOLUTION_MODE, resolution_mode.get_main_value())
...
func _on_aspect_ratios_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, ASPECT_RATIOS, aspect_ratios.get_main_value())
...
func _on_resolutions_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, RESOLUTIONS, resolutions.get_main_value())
...
#func _on_vertical_sync_change(toggled_on: bool) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, VERTICAL_SYNC, vertical_sync.get_main_value())
#
#
### 视野
#func _on_fov_change(value: float) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, FOV, fov.get_main_value())
#
#
### 曝光
#func _on_exposure_change(value: float) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, EXPOSURE, exposure.get_main_value())
#
#
### 各向异性过滤
#func _on_anisotropic_filtering_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, ANISOTROPIC_FILTERING, anisotropic_filtering.get_main_value())
#
#
### 多重采样抗锯齿2D
#func _on_msaa_2d_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, MSAA_2D, msaa_2d.get_main_value())
#
#
### 多重采样抗锯齿3D
#func _on_msaa_3d_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, MSAA_3D, msaa_3d.get_main_value())
#
#
### 屏幕空间抗锯齿
#func _on_ssaa_change(index: int) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, SSAA, ssaa.get_main_value())
#
#
### 时间抗锯齿
#func _on_taa_change(toggled_on: bool) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, TAA, taa.get_main_value())
#
#
### 屏幕空间反射
#func _on_ssr_change(value: float) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, SSR, ssr.get_main_value())
#
#
### 屏幕空间环境光遮蔽
#func _on_ssao_change(toggled_on: bool) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, SSAO, ssao.get_main_value())
#
#
### 屏幕空间间接照明
#func _on_ssil_change(toggled_on: bool) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, SSIL, ssil.get_main_value())
#
#
### 辉光
#func _on_glow_change(toggled_on: bool) -> void:
	#CoreSystem.get("config_manager").set_value(GRAPHICS, GLOW, glow.get_main_value())
