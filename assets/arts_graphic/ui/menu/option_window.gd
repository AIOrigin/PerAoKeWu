...
func _on_close_requested() -> void:
	SettingsConfigManager.load_compare_config(CoreSystem.get("config_manager").config_path)
	for section in CoreSystem.get("config_manager")._config_file.get_sections():
		for key in section:
			if SettingsConfigManager._compare_config_file.get_value(section, key) != CoreSystem.get("config_manager")._config_file.get_value(section, key):
				show()
				in_setting = true
